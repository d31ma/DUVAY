...
test('homepage composition dogfoods DuVay components and utilities', async () => {
  const markup = await readFile(projectFile('website/client/pages/tac.html'), 'utf8');
...
test('site header dogfoods the DuVay application and button surfaces', async () => {
  const markup = await readFile(projectFile('website/client/components/site/header/tac.html'), 'utf8');

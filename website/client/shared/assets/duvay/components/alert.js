 ...
 *   icon-size     - CSS length, or one of the named sizes (x-small | small |
 *                   default | large | x-large) resolved through `icon-sizes`
 *   icon-sizes    - JSON array of [name, px] tuples overriding the named sizes,
 *                   e.g. '[["small",12],["large",44]]'
...
class WAlert extends WElement {
  ...
  // Vuetify's default icon size scale, overridable per alert via `icon-sizes`.
  static iconSizes = [['x-small', 10], ['small', 16], ['default', 24], ['large', 28], ['x-large', 40]];
  ...
  get iconSize()    { return this._resolveIconSize(this._attr('icon-size', '')); }
  ...
  _template() {
    if (!this.active) return this._inactiveMarkup();
    ...
    let html = `<div class="${this._classes(type, borderSide)}" role="alert" aria-live="${live}"${styles}>`;
    ...
    html += this._bodyMarkup();
    html += '<slot></slot></div>';
    html += this._appendMarkup();
  ...
  _appendMarkup() {
    if (!this._hasSlot('append')) return '';
    return '<div class="w-alert-append"><slot name="append"></slot></div>';
  }
  ...
  // `icon-sizes` is a list of [name, px] tuples; entries that are not a
  // name/number pair are ignored so a malformed override cannot break rendering.
  _iconSizeMap() {
    const map = new Map(this.constructor.iconSizes);
    let parsed;
    try { parsed = JSON.parse(this.getAttribute('icon-sizes') || '[]'); } catch (_) { return map; }
    if (!Array.isArray(parsed)) return map;
    parsed.forEach((entry) => {
      if (!Array.isArray(entry) || !entry[0] || !Number.isFinite(Number(entry[1]))) return;
      map.set(String(entry[0]), Number(entry[1]));
    });
    return map;
  }

  // A named size resolves through the map; anything else is already a CSS length.
  _resolveIconSize(value) {
    if (!value) return '';
    const px = this._iconSizeMap().get(value);
    return px === undefined ? value : px + 'px';
  }

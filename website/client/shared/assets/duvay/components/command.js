...
export class WCommand extends WElement {
  ...
  _renderItem(item) {
    ...
    const row = this._groupRow(item);
    if (row) return row;
    const title = item.title != null ? item.title : (item.text != null ? item.text : '');
    return `<w-command-item ${this._itemAttrs(item)}>${this._esc(title)}</w-command-item>`;
  }
  ...
  _matchHotkey(event) {
    ...
    if (!this._matchMods(parts.slice(0, -1), event)) return false;
    return event.key.toLowerCase() === key;
  }

  // Unlisted names are ignored, so only the modifiers named in the spec are
  // required to be held.
  _matchMods(mods, event) {
    const held = {
    ...
    return mods.every((name) => held[name] !== false);
  }
}

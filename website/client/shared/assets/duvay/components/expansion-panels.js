...
export class WExpansionPanels extends WElement {
  ...
  // Panel-level presentation the group can set once for every child. A panel
  // that names the attribute itself always wins.
  static cascadeAttrs = ['static', 'hide-actions', 'expand-icon', 'collapse-icon', 'ripple', 'hover', 'focusable', 'selected-class'];
  ...
  get max() {
    ...
    return this.hasAttribute('max') && Number.isFinite(value) ? value : Infinity;
  }
  get variant() {
  ...
  _cascade() {
    const panels = this._getItems();
    WExpansionPanels.cascadeAttrs.forEach((name) => {
      if (!this.hasAttribute(name)) return;
      const value = this.getAttribute(name);
      panels.forEach((panel) => {
        if (!panel.hasAttribute(name)) panel.setAttribute(name, value);
      });
    });
  }

  // `max` caps concurrent open panels: the one that just opened is the one
  // that gets rolled back.
  _enforceMax(toggledItem) {
    if (!toggledItem.open) return false;
    if (this._getItems().filter((item) => item.open).length <= this.max) return false;
    toggledItem.setOpen(false, true);
    return true;
  }
  ...
  _onItemToggle(toggledItem) {
    ...
    if (this._enforceMax(toggledItem)) return;

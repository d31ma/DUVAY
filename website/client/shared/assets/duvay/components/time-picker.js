...
export class WTimePicker extends WElement {
  static attrs = [
    'value', 'label', 'title', 'view', 'view-mode', 'format', 'period',
    'use-seconds', 'scrollable', 'allowed-hours', 'allowed-minutes', 'allowed-seconds',
    'min', 'max', 'color', 'elevation', 'width', 'density', 'hide-header', 'hide-title',
  ...
  // `hide-header` is DuVay's original spelling; Vuetify calls the same region
  // the picker title, so `hide-title` is accepted as an alias.
  get hideHeader() { return wBoolAttr(this, 'hide-header'); }
  get hideTitle() { return wBoolAttr(this, 'hide-title'); }
  ...
  _rootClasses() {
    return [
      ...
      this.divided ? 'w-time-picker--divided' : '',
  ...
  _headerTitle() {
    ...
    return this.ampmInTitle && this.format === 'ampm'
      ? `${title} ${this._period()}`
      : title;
  }

  _unitLabel() {
    if (this.view === 'hours') return 'hour';
    return this.view === 'seconds' ? 'second' : 'minute';
  }

  _clockItems(parts) {
    if (this.view === 'hours') return this._hourItems(parts);
    return this._unitItems(this.view === 'seconds' ? parts.second : parts.minute, parts);
  }

  _periodToggle(readonly) {
  ...
  _template() {
    ...
    return `<div class="${this._rootClasses()}"${style ? ` style="${style}"` : ''} aria-label="${this._esc(this.label)}">
      ${this.hideHeader || this.hideTitle ? '' : `<div class="w-time-picker-title">${this._esc(this._headerTitle())}</div>`}
      ...
      <div class="w-time-picker-header">
        ...
        ${this._periodToggle(readonly)}
      </div>
      <div class="w-time-picker-clock" role="listbox" aria-label="Choose ${this._unitLabel()}"${this.scrollable ? ' data-time-scrollable' : ''}>
        ${this._clockItems(parts)}
  ...
  // 'hours' | 'minutes' | 'seconds' view name to its singular unit.
  _unitFor(view) {
    if (view === 'hours') return 'hour';
    return view === 'minutes' ? 'minute' : 'second';
  }

  _unitValue(unit, parts) {
    if (unit === 'hour') return parts.hour;
    return unit === 'minute' ? parts.minute : parts.second;
  }

  // Highest value a typed field accepts before clamping.
  _fieldLimit(view) {
    if (view !== 'hours') return 59;
    return this.format === 'ampm' ? 12 : 23;
  }

  _fieldHour(value) {
    return this.format === 'ampm' ? to24Hour(value || 12, this.period) : value;
  }

  _setField(view, rawValue) {
    if (this.disabled || this.readonly) return;
    const parts = this._parts();
    const value = Math.max(0, Math.min(this._fieldLimit(view), parseInt(rawValue, 10) || 0));
    if (view === 'hours') parts.hour = this._fieldHour(value);
    if (view === 'minutes') parts.minute = value;
    if (view === 'seconds') parts.second = value;
    const unit = this._unitFor(view);
  ...
  _onFieldKey(event, view) {
    ...
    const unit = this._unitFor(view);
    const parts = this._parts();
    const next = this._nextAllowed(unit, this._unitValue(unit, parts), event.key === 'ArrowUp', parts, 1);
  ...
  _onWheel(event) {
    ...
    event.preventDefault();
    const parts = this._parts();
    const unit = this._unitFor(this.view);
    const next = this._nextAllowed(unit, this._unitValue(unit, parts), event.deltaY < 0, parts);
    if (unit === 'hour') return this._commitAbsoluteHour(next, parts);
    this._select(unit, next);
  }
  ...
  _commit(parts) {
    ...
    this._render();
    this._events();
    if (changed) this._emit('change', { value });
  }
...
// Shared with <w-time-picker-controls> and <w-time-picker-clock>, which model
// the same clock arithmetic on their own.
export {
  parseTime as wParseTime,
  pad as wPad,
  timeValue as wTimeSeconds,
  parseLimit as wParseLimit,
  to24Hour as wTo24Hour,
};

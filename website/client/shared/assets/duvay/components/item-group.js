...
export class WItemGroup extends WElement {
      ...
      this._observer = new MutationObserver(() => {
        ...
        if (slot) {
          Array.from(this.children).forEach((child) => {
            // Subclasses may wrap the group (w-slide-group adds a shell with
            // arrows), so skip ancestors of the group as well as its contents.
            if (child !== group && !group.contains(child) && !child.contains(group)) {
  ...
  _onGroupKeydown(event) {
    ...
    const move = WItemGroup.navKeys[event.key];
    ...
    this._focusItem(this._navTarget(items, move));
  }

  // The item a navigation key moves focus to: an edge for Home/End, otherwise
  // one step from the focused item (or the first item when focus is outside).
  _navTarget(items, move) {
    if (move === 'home') return items[0];
    if (move === 'end') return items[items.length - 1];

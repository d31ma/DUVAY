 ...
 *   active       - false hides only the badge bubble
 *   model-value  - Vuetify alias for active; false hides the badge bubble
 ...
 *   transition   - entry transition for the bubble: fade | scale | slide-y,
 *                  or false/none to disable it
 *
 * Slots:
...
class WBadge extends WElement {
  ...
  get active() {
    // `model-value` (Vuetify) and `active` both toggle the badge bubble.
    const attr = this.hasAttribute('model-value') ? 'model-value'
  ...
  _badgeClass() {
    ...
    return [
      ...
      wTransitionClass(this.getAttribute('transition'), 'w-badge'),
    ].filter(Boolean).join(' ');
  }

...
export function wSafeUrl(value) {
  const raw = String(value == null ? '' : value).trim();
  if (!raw || /[\u0000-\u001f\u007f]/.test(raw)) return '';
  try {
    const protocol = new URL(raw, 'https://duvay.invalid/').protocol.toLowerCase();
...
// Images use the same web policy, plus browser-minted blob URLs and a narrow
// raster-data allowlist. SVG and arbitrary data payloads stay rejected.
export function wSafeImageUrl(value) {
  const raw = String(value == null ? '' : value).trim();
  if (!raw || /[\u0000-\u001f\u007f]/.test(raw)) return '';
  if (/^blob:https?:\/\//i.test(raw)) return raw;
  if (/^data:image\/(?:avif|gif|jpeg|png|webp);base64,[a-z0-9+/]+={0,2}$/i.test(raw)) return raw;
  return wSafeUrl(raw);
}

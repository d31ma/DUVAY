...
class WElement extends HTMLElement {
  ...
  // Class and attribute strings as data rather than ternary chains. Falsy
  // entries drop out, so `_cls({ 'w-btn--block': this.block })` replaces
  // `this.block ? ' w-btn--block' : ''`. Computed keys cover value-suffixed
  // modifiers: `_cls({ ['w-btn--' + this.size]: this.size })`.
  _cls(map) {
    return Object.keys(map)
      .filter((name) => map[name] && name)
  ...
  // ` name="value"` per truthy entry; `true` renders a bare boolean attribute.
  // Values are escaped, so callers pass raw strings.
  _attrs(map) {
    return Object.keys(map).filter((name) => map[name]).map((name) => (
      map[name] === true ? ' ' + name : ` ${name}="${this._esc(map[name])}"`
    )).join('');
  }
  ...
  _commonClasses() {
    const classes = [];
    WElement.classAttrs.forEach(([attr, prefix]) => {
    ...
    WElement.flagClasses.forEach(([attr, name]) => {
      if (this.hasAttribute(attr)) classes.push(name);
    });
    return classes;
  }

  _applySizeProps() {
    WElement.sizeProps.forEach((name) => {
      const value = this.getAttribute(name);
      const prop = '--w-prop-' + name;
      if (value) this.style.setProperty(prop, value);
      else this.style.removeProperty(prop);
      this.classList.toggle('w-has-' + name, this.hasAttribute(name));
    });
  }

  // `theme` scopes a subtree to a palette; themes.css keys off `w-theme`, so
  // the host mirrors one onto the other. A w-theme the author wrote by hand
  // always wins, and is detected once, before the first mirror happens.
  _applyTheme() {
    const theme = this.getAttribute('theme');
    if (this._wThemeAuthored === undefined) {
      this._wThemeAuthored = this.hasAttribute('w-theme') && this.getAttribute('w-theme') !== theme;
    }
    if (this._wThemeAuthored) return;
    if (theme) this.setAttribute('w-theme', theme);
    else this.removeAttribute('w-theme');
  }

  // Sizing attributes every component accepts; each maps to a --w-prop-<name>
  // custom property plus a `w-has-<name>` class that consumes it.
  static sizeProps = ['width', 'height', 'min-width', 'max-width', 'min-height', 'max-height'];

  // Value attributes that become `<prefix><value>` classes.
  static classAttrs = [
    ['color', 'text-'],
    ['bg-color', 'bg-'],
    ...
    ['density', 'w-density-'],
    ['elevation', 'elevation-'],
    ['hover-elevation', 'w-hover-elevation-'],
    ['location', 'w-location-'],
    ['position', 'w-position-'],
  ];

  // Bare attributes that become a fixed class.
  static flagClasses = [
    ['border', 'w-border'],
    ['tile', 'rounded-0'],
    ['loading', 'w-common-loading'],
    ['readonly', 'w-common-readonly'],
    ['disabled', 'w-common-disabled'],
  ];

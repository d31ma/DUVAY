 ...
 *   mobile-breakpoint - px width, or xs|sm|md|lg|xl|xxl, below which the banner
 *                stacks automatically; it restacks as the viewport crosses it
...
class WBanner extends WElement {
  static attrs = ['text', 'icon', 'avatar', 'lines', 'sticky', 'mobile', 'stacked', 'color', 'bg-color', 'mobile-breakpoint'];
  ...
  // Forced by `stacked` / `mobile`, or implied below `mobile-breakpoint`.
  get stacked() { return this._bool('stacked') || wMobileMode(this); }

  _rerender() { this._render(); this._events(); }

  _events() { wWatchMobile(this); }

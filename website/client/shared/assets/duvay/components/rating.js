...
export class WRating extends WElement {
  ...
  get value() {
    // `model-value` (Vuetify) takes precedence over the legacy `value` attribute.
  ...
  // Arrow keys step by one rating unit (half a unit with half-increments);
  // Home/End jump to an edge. Any other key is left to the browser.
  static keySteps = { ArrowRight: 1, ArrowUp: 1, ArrowLeft: -1, ArrowDown: -1, Home: 'min', End: 'max' };

  _onKeydown(event) {
    if (this._keyBlocked(event)) return;

    const move = WRating.keySteps[event.key];
    if (move === undefined) return;

    event.preventDefault();
    this._commit(this._keyValue(move), true);
  }
  ...
  _keyValue(move) {
    if (move === 'min') return 0;
    if (move === 'max') return this.length;
    return this.value + move * (this.halfIncrements ? 0.5 : 1);
  }
  ...
  _commit(nextValue, moveFocus = false) {
    ...
    if (this.hasAttribute('model-value')) this._silentSet('model-value', formattedValue);
  ...
  _stripCloneIds(fragment) {
    if (fragment.removeAttribute) fragment.removeAttribute('id');

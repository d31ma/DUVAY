 ...
 *
 * Attributes:
 *   text/model-value/selected/value - content, visibility, and selection state
 ...
 *
 * Events:
 *   w-change, w-remove, click:close, update:model-value
 */
...
class WChip extends WElement {
  ...
  get active() {
    ...
    return !['false', '0', 'off', 'hidden'].includes(String(model).toLowerCase());
  }
  ...
  _toggleSelected() {
    ...
    this._dispatch('w-change', { selected, value: this.value });
  }

  _close(event) {
    this._dispatch('click:close', { value: this.value });
    this._dispatch('w-remove', { value: this.value });
    if (this.removable) {
      this.remove();
      return;
    }
    this._silentSet('model-value', 'false');
    this._emit('update:model-value', { value: false });
    this._render();

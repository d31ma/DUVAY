...
export class WDatePickerHeader extends WElement {
  ...
  _events() {
    this._q('.w-date-picker-header')?.addEventListener('click', (event) => {
      if (event.target.closest('[data-append]')) return;
      this._emit('w-click', {});
    });
    this._q('[data-append]')?.addEventListener('click', () => this._emit('w-click-append', {}));
  }
}

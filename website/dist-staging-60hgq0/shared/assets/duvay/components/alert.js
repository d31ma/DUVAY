 ...
 *   model-value   - false hides the alert
 *
 * Slots:
 *   default, prepend, title, text, append, close
 *
 * Events:
 *   w-close, w-dismiss, update:model-value
 */
...
class WAlert extends WElement {
  ...
  get active() {
    const state = this.getAttribute('model-value') ?? this.getAttribute('value') ?? this.getAttribute('open');
    if (state == null) return true;
    return !['false', '0', 'off', 'closed', 'hidden'].includes(String(state).toLowerCase());
  }
  ...
  _close() {
    ...
    this._silentSet('value', 'false');
    this._render();
    if (typeof this._events === 'function') this._events();
    this._applyCommonProps();

    this._emit('w-close', { value: false });
    this.dispatchEvent(new CustomEvent('w-dismiss', {
      detail: { value: false },
      bubbles: true,
      composed: true,
    }));

 ...
 *
 * Attributes:
 *   checked        - whether checked (use when not binding with model-value)
 ...
 *   true-value     - value to emit when checked (default: 'true')
 *   false-value    - value to emit when unchecked (default: 'false')
 *   model-value    - bound value; checked when model-value === true-value
 ...
 *
 * Events:
 *   w-change        - fires on toggle (detail: { checked, value, name })
 *   update:model-value - dispatched automatically when w-change carries .value
 */

class WCheckbox extends WElement {

  static attrs = ['checked', 'indeterminate', 'disabled', 'readonly', 'name', 'value', 'true-value', 'false-value', 'label', 'color', 'size', 'hint', 'error', 'hide-details'];
  ...
  get trueValue()     { return this._attr('true-value', 'true'); }
  get falseValue()    { return this._attr('false-value', 'false'); }
  ...
  _deriveChecked() {
    ...
    if (model !== null) return model === this.trueValue;
  ...
  _events() {
    ...
    inp.addEventListener('change', () => {
      ...
      const modelValue = checked ? this.trueValue : this.falseValue;
      const value = this.hasAttribute('model-value') ? modelValue : this.value;

      if (this.hasAttribute('model-value')) {
        this._silentSet('model-value', modelValue);
        this._silentSet('checked', null);
      } else {
        this._silentSet('checked', checked);
      }
      this._silentSet('indeterminate', false);

      this._emit('w-change', { checked, indeterminate: false, name: this.name, value });
    });
  }
}

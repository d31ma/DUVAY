 ...
 *
 * Attributes:
 *   value / model-value - selected value (comma-joined when multiple)
 ...
 *
 * Events:
 *   w-change            - { value, name } on selection change
 *   update:model-value  - { value } mirror (from the base class)
 */
...
class WSelect extends WElement {
  static attrs = ['value', 'model-value', 'disabled', 'readonly', 'name', 'label', 'hint',
  ...
  _events() {
    ...
    if (!this.__chipListeners) {
      ...
      this.addEventListener('w-remove', onRemove);
      this.addEventListener('click:close', onRemove);
    }
  ...
  _commit(values, { keepOpen = false } = {}) {
    ...
    this._emit('w-change', { value: next, name: this.name });
  }

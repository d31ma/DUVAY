...
export class WDatePicker extends WElement {
  ...
  _selectMonth(month) {
    ...
    this._setView('date');
    this._emit('w-view-change', { month, year: this.year, view: 'date' });
  }

  _selectYear(year) {
    this._silentSet('year', year);
    this._setView('months');
    this._emit('w-view-change', { month: this.month, year, view: 'months' });
  }
  ...
  _shiftView(delta) {
    ...
    this._render();
    this._events();
    this._emit('w-view-change', { month: this.month, year: this.year, view: this.view });
  }
  ...
  _onDayKeyDown(event, button) {
    ...
    if (!nextButton) {
      ...
      this._render();
      this._events();
      this._emit('w-view-change', { month: next.getMonth() + 1, year: next.getFullYear(), view: this.view });
  ...
  _select(date) {
    ...
    this._render();
    this._events();
    this._emit('w-change', { value: nextValue });
  }
}

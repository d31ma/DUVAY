 ...
 *   value / model-value - selected value(s); comma-separated when multiple
 ...
 *
 * Events:
 *   w-input   - search query changed (detail: { value, name })
 *   w-select  - an item was chosen (detail: { value, title })
 *   w-change  - value changed (detail: { value, name })
 *   w-clear   - clear button clicked (detail: { name })
 *   update:model-value - emitted automatically when value changes
 */
...
export class WAutocomplete extends WElement {
  static attrs = [
    'label', 'items', 'item-title', 'item-value', 'value', 'model-value',
  ...
  get value() {
    return this._value !== undefined ? this._value : this._attr('value', this._attr('model-value', ''));
  }
  set value(v) {
    this._value = v;
    this._silentSet('value', v);
    this._silentSet('model-value', v);
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    ...
    if (name === 'value' || name === 'model-value') {
  ...
  _events() {
    ...
    input.addEventListener('input', () => {
      ...
      this._filter();
      this._syncClear();
      this._emit('w-input', { value: input.value, name: this.name });
    });
  ...
  _clear() {
    ...
    this._filter();
    this._closeList();
    this._emit('w-change', { value: '', name: this.name });
    this._emit('w-clear', { name: this.name });
  ...
  _selectItem(item) {
    ...
    this._emit('w-select', { value: this.value, title });
    this._emit('w-change', { value: this.value, name: this.name });
  }
  ...
  _commitFreeText(text) {
    ...
    this._emit('w-select', { value: this.value, title: text });
    this._emit('w-change', { value: this.value, name: this.name });
  }

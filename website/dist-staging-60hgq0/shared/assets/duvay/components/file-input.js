 ...
 *
 * Events:
 *   w-change        - fires when files change (detail: { files, value })
 *   w-input         - fires on every file selection (detail: { files, value })
 *   click:clear      - fires when the clear button is clicked (detail: { files: [], value: [] })
 */
...
export class WFileInput extends WElement {
  ...
  _events() {
    ...
    if (input) {
      input.addEventListener('change', (event) => {
        ...
        this._events();
        this._applyCommonProps();
        this._emitFiles('w-change');
        this._emitFiles('w-input');
      });
  ...
  _clear() {
    ...
    this._events();
    this._applyCommonProps();
    this._emit('click:clear', { files: [], value: [] });
    this._emit('w-change', { files: [], value: [] });
  }
}

 ...
 *
 * Attributes:
 *   value / model-value  - selected numeric value
...
export class WRating extends WElement {
  ...
  get value() {
    ...
    const source = modelValue != null ? modelValue : this._attr('value', '0');
    return this._clamp(this._number(source, 0));
  }

  set value(nextValue) {
    this.setAttribute('value', this._formatValue(this._clamp(nextValue)));
  }
  ...
  _colorValue(color, fallback) {
    ...
    if (TOKEN_COLORS.has(value)) return `var(--w-${value})`;
    if (/^(#[0-9a-f]{3,8}|rgba?\([\d\s,.%]+\)|hsla?\([\d\s,.%]+\))$/i.test(value)) return value;
    return fallback;
  }
}

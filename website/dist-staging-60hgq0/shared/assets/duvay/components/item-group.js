...
import { wBoolAttr, wModelValue, wNumberAttr, wValueList } from './utils.js';

export class WItemGroup extends WElement {
  static attrs = ['value', 'model-value', 'multiple', 'mandatory', 'max', 'selected-class', 'active-class', 'disabled'];
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    ...
    if (this._rendered) this._syncGroupItems();
  }
  ...
  _setSelected(selected) {
    const value = this.multiple ? JSON.stringify(selected) : (selected[0] || '');
    if (value === this.value && value === this.getAttribute('model-value')) return;
    this._silentSet('value', value || null);
    this._silentSet('model-value', value || null);
    this._syncGroupItems();
    this._emit('w-change', { value: this.multiple ? selected : value });
  }

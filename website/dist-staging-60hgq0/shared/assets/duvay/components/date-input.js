 ...
 *
 * Events:
 *   w-input  - fires while typing (detail: { value, name })
 *   w-change - fires when the value commits (detail: { value, name })
 */
import {
  wIsoDate,
  wParseIsoDate,
...
export class WDateInput extends WElement {
  ...
  _events() {
    ...
    if (picker) {
      picker.addEventListener('w-change', (e) => {
        e.stopPropagation();
  ...
  _commit(nextValue) {
    ...
    if (changed) this._emit('w-change', { value: nextValue, name: this.name });
  }

...
export class WChipGroup extends WElement {
  static attrs = ['value', 'model-value', 'multiple', 'mandatory', 'disabled', 'selected-class'];
  ...
  _events() {
    ...
    this.addEventListener('w-change', this.__wChipGroupChange);
  }
  ...
  _commit(values) {
    ...
    this._silentSet('model-value', next || null);
    this._syncChildren(values);
    this._emit('w-change', { value: this.multiple ? values : next });
  }

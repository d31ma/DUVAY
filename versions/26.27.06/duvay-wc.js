...
(function(root) {
  ...
  function init(rootNode) {
    ...
    scope.querySelectorAll("[w-tween]").forEach((el) => {
      if (el.hasAttribute("w-tween-ready"))
        return;
      el.setAttribute("w-tween-ready", "1");
      if (el.getAttribute("w-tween-auto") !== "false")
        tween(el);
    });
    scope.querySelectorAll("[w-spring]").forEach((el) => {
      if (el.hasAttribute("w-spring-ready"))
        return;
      el.setAttribute("w-spring-ready", "1");
      if (el.getAttribute("w-spring-auto") !== "false")
        spring(el);
    });
  }
    ...
    if (this._rendered) {
      ...
      if (content) {
        Array.from(content.childNodes).forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE && node.hasAttribute("w-tooltip-generated"))
            return;
    ...
    if (drawer) {
      drawer.addEventListener("click", (event) => {
        if (!this.closeOnNavigation || !this._isCompact())
          return;
        if (event.target.closest("a[href], [w-drawer-close]"))
...
class WNumberInput extends WElement {
  ...
  _template() {
    ...
    const ph = this.placeholder ? ` placeholder="${this._esc(this.placeholder)}"` : "";
    const nm = this.name ? ` name="${this._esc(this.name)}"` : "";
    const input = `<input class="w-input${sizeClass}" type="text" inputmode="decimal"` + ` value="${this._esc(this._format(this.model))}"${minAttr}${maxAttr} step="${this.step}"${ph}${nm}` + `${this.disabled ? " disabled" : ""}${this.readonly ? " readonly" : ""} w-number-input>`;
...
class WSnackbar extends WElement {
  ...
  _template() {
    ...
    const actions = this._hasSlot("actions") ? '<slot name="actions"></slot>' : this.action ? `<button class="w-snackbar-action" type="button" w-snackbar-action>${this._esc(this.action)}</button>` : "";
